...
sap-transactions-reference.md - Quick lookup of SAP transaction codes, modules, and data tables
example-p2p-fs.md - Complete example FS for Procure-to-Pay (shows final output quality)
quick-reference.md - User-friendly guide to using the skill
...
Gather Business Context - Clarifying questions, scope definition
Explore FS Context & Structures - SAP modules, transaction codes
Data Model Mapping - Master data relationships (text + visual)
Propose Approaches - 2-3 options with detailed pros/cons
Write Formal FS - Complete document with all standard sections
User Review & Iteration - Inline comments, change tracking
Implementation Transition - Roadmap to development phase
...
Let the skill ask clarifying questions
...
Typical Timeline
StageEffortUser Input1. Gather Context15 minDescribe requirement, answer questions2. Explore Context20 minProvide clarifications if needed3. Data Model20 minReview/approve diagram4. Propose Approaches30 minProvide feedback on options5. Write FS60 minPassive (skill generates document)6. Review & Iterate30-60 minReview sections, request changes7. Transition Plan30 minReview implementation roadmapTOTAL3-4 hoursInteractive process
...
✅ Transaction code mapping (ME51N, ME21N, MIGO, MIRO, F-53)
✅ Master data model (MARA → MARC → MARD, LFA1 → LFB1 → LFC1)
...
✅ 50-page FS document with all sections
...
Key Advantages
AspectBenefitSpeedGenerate complete FS in 3-4 hours (vs. 2-3 weeks manual)CompletenessNothing missed (validation checklist ensures 20+ key items)QualityProfessional, enterprise-grade documentationFlexibilityIterative review process accommodates feedbackKnowledgeIncorporates best practices from 100+ SAP implementationsSAP-ReadyAll recommendations align with SAP standardsTraceableVersion history & change log for governanceScalableWorks for any SAP business process (P2P, O2C, etc.)
Getting Started

Read SKILL.md - Understand the 7-stage workflow
Skim example-p2p-fs.md - See what final output looks like
Keep quick-reference.md handy - Refer to during interactions
Use sap-transactions-reference.md - Lookup table for SAP codes
